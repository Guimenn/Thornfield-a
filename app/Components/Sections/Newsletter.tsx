...
export default function Newsletter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        // Mostrar alert de email já cadastrado
        alert(`Este email ${email} já está inscrito em nossa newsletter.`);
...
const SubscriptionForm = ({ 
  ...
  const cadastrarNewsletter = async () => {
    ...
    try {
      ...
      // Feedback de sucesso
      showStyledFeedback(true, "Inscrição realizada com sucesso. Bem-vindo à nossa newsletter!");
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div 
            className={`max-w-md w-full rounded-xl shadow-2xl p-6 ${
              isSuccess ? 'bg-gradient-to-br from-amber-900/90 to-amber-800/90' : 'bg-gradient-to-br from-red-900/90 to-red-800/90'
            } backdrop-blur-md border ${
              isSuccess ? 'border-amber-600/50' : 'border-red-700/50'
  ...
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
    <div 
      className={`max-w-md w-full rounded-xl shadow-2xl p-6 ${
        ...
        feedback.type === 'success' ? 'border-green-700' : 'border-red-700'
      } transform transition-all`}
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center">
